...
# 你的刚性流动性过滤门槛（20日平均成交额大于 1000 万，自动屏蔽规模过小的僵尸ETF）
MIN_AVG_AMOUNT_20 = 10_000_000

# 严格校对后的 300+ 只核心主流 ETF 监测池（上海.SS / 深圳.SZ 已全部修正）
ETF_WATCHLIST = {
    # --- 核心宽基指数 ---
    ...
    "159690.SZ": "红利低波100ETF", "510650.SS": "金融ETF",
    ...
    "159619.SZ": "数字经济ETF", "159620.SZ": "基建50ETF", "516070.SS": "工业有色ETF",
}
...
def get_etf_data_and_analyze():
    ...
    for symbol, name in ETF_WATCHLIST.items():
        try:
            # 移除了极其缓慢且不稳定的 info 接口，直接提取高开低收历史日线
            ...
            # 刚性过滤：20日均成交额低于 1000 万的直接在第一步干掉（变相过滤了小市值僵尸基金）
...
def run():
    ...
    if result_df.empty:
        msg = "今日精选主流名单中，没有同时符合20日均成交额>1000万且满足MA20/60右侧趋势的ETF。"
    else:
        ...
        msg = "数据源：Yahoo Finance (全面校正)\n"
        msg += "刚性过滤：20日平均每日成交额 > 1000 万元（自动过滤1亿以下小市值）\n"
